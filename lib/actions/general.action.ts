...
export async function createFeedback(params: CreateFeedbackParams) {
  ...
  try {
    ...
    // Generate structured feedback object
    const { object } = await generateObject({
      model: google("gemini-2.0-flash-001"), // ✅ no extra config here
      schema: feedbackSchema, // JSON schema for structured output
      prompt: `
    ...
    // Build feedback object for Firestore
    const feedback = {
      interviewId,
      userId,
    ...
    // Store in Firestore (create new or update existing)
...
export async function getFeedbackByNotInterviewId(
  params: GetFeedbackByInterviewIdParams
): Promise<Feedback | null> {
  const { interviewId, userId } = params;
